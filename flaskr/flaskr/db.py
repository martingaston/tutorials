...
def get_db():
    ...
    # either send along a new instance of g.db or forward it along
    return g.db  # g is the Flask global (g) application context


def close_db(e=None):
    """Close a sqlite3 database connection if one exists"""
    db = g.pop('db', None)  # gets and removes an attribute from g
...
def init_app(app):
    ...
    # we're using a factory function so there's no application instance available in this file - so we call these functions from within the factory
    app.teardown_appcontext(
        close_db)  # tell Flask to call a function after returning response
